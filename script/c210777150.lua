...
function s.filter1(c)
	return c:IsSetCard(0x14c) and c:IsType(TYPE_FUSION)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local typ=Duel.AnnounceRace(tp,1,0xffff)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local lvl=e:GetLabel()	--lvl
	local typ=Duel.GetChainInfo(0,CHAININFO_TARGET_PARAM) --race
