...
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and g:IsExists(s.thfilter,1,nil,tp) then
		...
		if Duel.IsExistingMatchingCard(Card.IsType,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,TYPE_TRAP) and Duel.SelectYesNo(p,aux.Stringid(id,3)) then
...
function s.lvcond(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(Card.IsType,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,TYPE_TRAP) 
end