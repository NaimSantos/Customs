...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 then
		...
		if c:IsRelateToEffect(e) and c:IsFaceup() and Duel.CheckPendulumZones(tp) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
			Duel.BreakEffect()
...
function s.selfedop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SendtoExtraP(c,tp,REASON_EFFECT)>0 then
		if Duel.IsExistingMatchingCard(s.polyfilter,tp,LOCATION_GRAVE,0,1,nil)
			and Duel.CheckPendulumZones(tp) and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then
