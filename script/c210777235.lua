...
function s.tedop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoExtraP(g,tp,REASON_EFFECT) and c:IsRelateToEffect(e) and Duel.CheckPendulumZones(tp) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		Duel.BreakEffect()
...
function s.selfedop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SendtoExtraP(c,tp,REASON_EFFECT)>0 then
		if Duel.IsExistingMatchingCard(s.tpzfilter,tp,LOCATION_HAND,0,1,nil) and Duel.CheckPendulumZones(tp) and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
